...
Converts broadband spectrum `level` over specified `bw` in Hz to overall level 
"""
sl_to_ol(level, bw) = level + 10*log10(bw)

""" 
    wiggle(s, field, value)

Copy struct `s`, adjusting value of `field` to `value`
"""
function wiggle(s, field::Symbol, value)
    T = typeof(s)
    fields = fieldnames(T)
