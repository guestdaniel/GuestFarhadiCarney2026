...
function gaussian_noise_sl(;
    ...
    bw = NaN
    if isempty(filter)
        # Branch based on input arguments
        if isnan(freq_low)
            # If freq_low is nan, we want a lowpass filter (setting cutoff to freq_high)
            filter = digitalfilter(Lowpass(freq_high; fs=fs), Butterworth(8))
            bw = fs-freq_high
        elseif isnan(freq_high)
            # If freq_high is nan, we want a highpass filter (setting cutoff to freq_low)
            filter = digitalfilter(Highpass(freq_low; fs=fs), Butterworth(8)) 
            bw = freq_low
        else
            # Otherwise, we want a bandpass filter
            filter = digitalfilter(Bandpass(freq_low, freq_high; fs=fs), Butterworth(8))
            bw = freq_high-freq_low
        end
    end

    # Generate random numbers
    waveform = randn(samples(dur, fs))
    
    # Apply filter to waveform
    waveform = filt(filter, waveform)

    # Scale to correct overall level based on spectrum level; note that
    # this scaling assumes the noise is perfectly bandlimited to the 
    # specified range, which is not true in practice (but still reasonably close)
    scale_dbspl!(waveform, sl_to_ol(level, bw))
