...
// 	int    nrep, pxbins, lp, outsize[2], totalstim;
	...
	mwSize outsize[2]; /* DMS, 1 Aug 2019 */
	...
	if (nrhs != 8) 
	{
		mexErrMsgTxt("model_Synapse requires 8 input arguments (normally 7, but we added sampFreq).");
	}; 
	...
	SingleAN(px,cf,nrep,tdres,totalstim,fibertype,noiseType,implnt,sampFreq,meanrate,varrate,psth);

 mxFree(px);

}

void SingleAN(double *px, double cf, int nrep, double tdres, int totalstim, double fibertype, double noiseType, double implnt, double sampFreq, double *meanrate, double *varrate, double *psth)
{	
...
{    
   ...
   /*----------------------------------------------------------*/
    ...
    /* DRG 08/01/2023 --- Quick approximate power law for Peter */
    /* NOT FOR GENERAL USE */
    ...
    double E1[100], E2[100], D1[100], D2[100];

    /* Calculate/initialize decay parameters for PLA approximation */
    double delta_slow = (log(tau_long_slow) - log(tau_short_slow)) / (100 - 1);
    double tau_temp_slow = 0.0;
    double delta_fast = (log(tau_long_fast) - log(tau_short_fast)) / (100 - 1);
    double tau_temp_fast = 0.0;
    for (int p = 0; p < 100; p++) {
        ...
        tau_temp_slow = exp(log(tau_short_slow) + delta_slow * p);
        D1[p] = exp(-(1/(1/tdres)) / tau_temp_slow);
        tau_temp_fast = exp(log(tau_short_fast) + delta_fast * p);
        D2[p] = exp(-(1/(1/tdres)) / tau_temp_fast);
    }
    ...
    for (int n=0; n<floor((totalstim*nrep+2*delaypoint)*tdres*sampFreq); n++)
    {
        if (implnt == 0) {
          ...
          sout2[k]  = __max( 0, sampIHC[n] - alpha2*I2); 
            ...
            sout1[n]  = __max(0, sampIHC[n] + randNums[n] - coef_slow*I1);
            sout2[n] = __max(0, sampIHC[n] - coef_fast*I2);
