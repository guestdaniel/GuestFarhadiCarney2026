...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double *px, cf, tdres, fibertype, noiseType, implnt, noisein, *state2in;
	int    nrep, pxbins, lp, totalstim, s;
        mwSize outsize[2], outsize2[2];
	double *noiseintmp, *pxtmp, *cftmp, *nreptmp, *tdrestmp, *fibertypetmp, *noiseTypetmp, *implnttmp, *state2intmp;
        
    double *meanrate, *varrate, *state2;  //afagh
   
	void   SingleAN(double *, double, int, double, int, double, double, double, double *, double *, double *, double *, double);
	...
	state2intmp = mxGetPr(prhs[7]);
	noiseintmp = mxGetPr(prhs[8]);
...
/* Calculate number of samples for total repetition time */
	...
	state2in =(double*)mxCalloc(6,sizeof(double)); 
	...
	outsize2[0]=1;
	outsize2[1]=6;
	plhs[0] = mxCreateNumericArray(2, outsize, mxDOUBLE_CLASS, mxREAL);     // ma fek konam baraye in feedback faghat ino niaz darim 
	...
	meanrate	  = mxGetPr(plhs[0]);   // pas faghat meanrate
    varrate = mxGetPr(plhs[1]);
    state2	  = mxGetPr(plhs[2]);
			
	/* run the model */

	//mexPrintf("ANmodel: Zilany, Bruce, Ibrahim, and Carney : Auditory Nerve Model\n");

	SingleAN(px,cf,nrep,tdres,totalstim,fibertype,noiseType,implnt,meanrate,varrate,state2,state2in,noisein); 
 mxFree(px);

}

void SingleAN(double *px, double cf, int nrep, double tdres, int totalstim, double fibertype, double noiseType, double implnt, double *meanrate, double *varrate, double *state2, double *state2in, double noisein)
{	
        	
	/*variables for the signal-path, control-path and onward */
	double *synouttmp,*sptime, *test, *state2tmp;    // exponout--> afagh
	double CIout[1], CLout[1], I2out[1], I1out[1],s1out[1],s2out[1],s1out2[1],s2out2[1];
	double n11[1],n21[1],n31[1],n12[1],n22[1],n32[1],m11[1],m21[1],m31[1],m41[1],m51[1],m12[1],m22[1],m32[1],m42[1],m52[1];
	int    i,nspikes,ipst;
	double I,spont;
    double sampFreq = 10e3; /* Sampling frequency used in the synapse */
        
    /* Declarations of the functions used in the program */
	double Synapse(double *, double, double, int , int , double , double , double , double , double *,   double *, double *,  double * , double * , double * , double *, double *,double *,double *,double *,double *,double *,double *,double *,double *,double *,double *,double *,double *,double *,double *,double *,double *, double);  // afagh akhario ezafe karde 
	...
	//exponOut = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));  //afagh
	test = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));  //afagh
	state2tmp   = (double*)mxCalloc(22,sizeof(double));
 ...
 I = Synapse(px, tdres, cf, totalstim, nrep, spont, noiseType, implnt,sampFreq, synouttmp   , CIout, CLout,     s1out, s2out, test,    s1out2, s2out2,     n11,n21,n31,n12,n22,n32,m11,m21,m31,m41,m51,m12,m22,m32,m42,m52, noisein); 
	...
	state2tmp[4]=s1out2[0];
	state2tmp[5]=s2out2[0];	
	/*
	state2tmp[6]=n11[0];	
	state2tmp[7]=n21[0];	
	state2tmp[8]=n31[0];	
	...
	*/
    ...
    for(i = 0; i<I ; i++)         // I fek konam totalsim*nrep bashe  if nrep==1 we do not need this I think
	{       
	...
	{       
		varrate[i] = synouttmp[i];///pow((1+0.75e-3*meanrate[i]),3); /* estimated instananeous variance in the discharge rate */  //we do not need this now
...
/* --------------------------------------------------------------------------------------------*/
double Synapse(double *ihcout, double tdres, double cf, int totalstim, int nrep, double spont, double noiseType, double implnt, double sampFreq, double *synouttmp,   double *CIout, double *CLout,  double * s1out, double * s2out, double *test , double *s1out2, double *s2out2,double *n11,double *n21,double *n31,double *n12,double *n22,double *n32,double *m11,double *m21,double *m31,double *m41,double *m51,double *m12,double *m22,double *m32,double *m42,double *m52, double noisein)  // afagh akhario ezafe karde 
{     
    /* Initalize Variables */     
    int z, b;
    ...
    double *sout1, *sout2, *exponOut;// *synSampOut;// *powerLawIn, *TmpSyn;    //exponOut inja tarif shode bood man avazesh kardam         
  ...
  //  powerLawIn = (double*)mxCalloc((long) ceil(totalstim*nrep+3*delaypoint),sizeof(double));
    sout1 = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));
    sout2 = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));
   // synSampOut  = (double*)mxCalloc((long) ceil((totalstim*nrep+2*delaypoint)*tdres*sampFreq),sizeof(double));
    ...
    m1 = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));
    m2 = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));
    m3  = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double)); 
    m4 = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));
    m5  = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double)); 
    
    n1 = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));
    n2 = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));
    n3 = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));  
	
    /*----------------------------------------------------------*/    
    /*------- Parameters of the Power-law function -------------*/
    /*----------------------------------------------------------*/ 
    binwidth = tdres; // I chnged it !!!
  ...
  //  randNums = mxGetPr(randOutputArray[0]);      
    ...
    /*----------------------------------------------------------*/    
       ...
       for (indx=0; indx<totalstim*nrep; ++indx)
       {
			...
			CIout[0]=CI;
			CLout[0]=CL;
            exponOut[k] =PPI*CI; //afagh
            k=k+1;
        }                 
  /*      for (k=0; k<delaypoint; k++)                                          //exponOut ro bayad bedim biroon va moghayese konim age yeki nabood yeki konim age bood bayad in ghesmat powerlaw ro say konim to matlab dorost konim 
			powerLawIn[k] = exponOut[0];                                            		//{	
        for (k=delaypoint; k<totalstim*nrep+delaypoint; k++)								//
			powerLawIn[k] = exponOut[k-delaypoint];											//in ghesmat power-law hast ke moshkel dar mishe
...
*/
  //  mxFree(powerLawIn); //mxFree(exponOut);  afagh 
   /*----------------------------------------------------------*/
   ...
   /*----------------------------------------------------------*/
	...
	I1=0;
	I2=0;
    for (indx=0; indx<totalstim*nrep; indx++) // afagh
    {
		  sout1[k]  = __max( 0, exponOut[indx] +noisein- alpha1*I1); //*/   /* No fGn condition */
          sout2[k]  = __max( 0, exponOut[indx] - alpha2*I2); 
		  
		     
              
                      I1 = (sout1[0])*binwidth/(beta1);
                      I2 = (sout2[0])*binwidth/(beta2);              
                k=k+1;
	...
	s1out[0]=sout1[0]; //s1in
	s2out[0]=sout2[0];//s2in
	s1out2[0]=sout1[1];//s1in2
	s2out2[0]=sout2[1];	//s2in2	
    ...
    /*----------------------------------------------------------*/        // inam fek konam moshkel bashe !!!!! fek konam kolan bayad bebarimesh to matlab faghat noktee ke mimone vase ine ke age second sample needs the output of first sample in any of the steps we need to consider this too. 
    /*for(z=0; z<k-1; ++z)
    {    
