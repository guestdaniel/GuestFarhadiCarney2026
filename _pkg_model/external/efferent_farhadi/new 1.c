...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	statein =(double*)mxCalloc(166,sizeof(double)); 
    ...
    outsize2[0] = 1;
	outsize2[1] = 166;   // change this when adding states
    outsize3[0]=1;
    outsize3[1]=1;
	plhs[0] = mxCreateNumericArray(2, outsize, mxDOUBLE_CLASS, mxREAL);
	plhs[1] = mxCreateNumericArray(2, outsize2, mxDOUBLE_CLASS, mxREAL);
	plhs[2] = mxCreateNumericArray(2, outsize3, mxDOUBLE_CLASS, mxREAL);
	/* Assign pointers to the outputs */
	
	ihcout = mxGetPr(plhs[0]);
	state  = mxGetPr(plhs[1]);	
    tauprint= mxGetPr(plhs[2]);	
	
	/* run the model */

	IHCAN(px,cf,nrep,tdres,totalstim,cohc,cihc,species,ihcout,state,statein,tauprint);
...
void IHCAN(double *px, double cf, int nrep, double tdres, int totalstim,
                double cohc, double cihc, int species, double *ihcout,  double *state, double *statein, double *tauprint)
{	
	...
	double *ihcouttmp,*tmpgain, *statetmp, *tautmp;
	int    grd;
	...
	statetmp   = (double*)mxCalloc(166,sizeof(double));
    tautmp= (double*)mxCalloc(1,sizeof(double));
...
statetmp[148]=expoafterfilter;
				...
				double alpha1, I1, alpha2, I2, sout1, sout2,n1,n2,n3,m1,m2,m3,m4,m5;
				 ...
				 sout1  = __max( 0, exponOut - alpha1*I1); //*/   /* No fGn condition */
  ...
  powerlawafterfilter = IhcLowPass(sout1,tdres,10000,0,1.0,7,Ihcstateout,Ihcin);     
		...
		for (int i=0; i<16 ; i++)
		{
			statetmp[i+150]=Ihcstateout[i];
                ...
                tauprint[0]= tmptauc1;
				statetmp[149]=powerlawafterfilter;
			////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
tauprint=tautmp;
    mxFree(ihcouttmp); mxFree(statetmp); mxFree(tautmp); 
